...
contract ve is IERC721, IERC721Metadata {
    ...
    string public override name = "veNUT NFT";
    string public override symbol = "veNUT";
    ...
    function increase_unlock_time(uint _tokenId, uint _lock_duration) external nonreentrant {
        ...
        require(unlock_time <= block.timestamp + 2*MAXTIME, 'Voting lock can be 8 years max');
    ...
    function _tokenURI(uint _tokenId, uint _balanceOf, uint _locked_end, uint _value) internal pure returns (string memory output) {
        ...
        output = string(abi.encodePacked(output, "veNUT: ", toString(_tokenId), '</text><text x="10" y="40" class="base">'));
        ...
        string memory json = Base64.encode(bytes(string(abi.encodePacked('{"name": "lock #', toString(_tokenId), '", "description": "veNUT NFT", "image": "data:image/svg+xml;base64,', Base64.encode(bytes(output)), '"}'))));
