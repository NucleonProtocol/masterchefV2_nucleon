...
contract MasterChefV2 is BoringOwnable, BoringBatchable {
    ...
    /// @notice LP sum of each MCV2 pool.
    uint256[] public PoolLPSum;
    ...
    modifier onlyStartDistributing() {
        require(block.timestamp > startDistributingTimestamp, 
    ...
    /// @param pid The index of the pool. See `poolInfo`.
    /// @param to Receiver of the LP tokens.
    function emergencyWithdraw(uint256 pid, address to) public onlyStartDistributing{
        UserInfo storage user = userInfo[pid][msg.sender];
